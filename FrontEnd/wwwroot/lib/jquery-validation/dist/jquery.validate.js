...
$.extend($.fn, {
    ...
    if (validator.settings.onsubmit) {
      ...
      this.on("submit.validate", function(event) {
        ...
        function handle() {
          ...
          // button The hidden input is inserted in two cases:
          //   - A user defined a `submitHandler`
          //   - There was a pending request due to `remote` method and
          //   `stopRequest()`
  ...
  rules : function(command, argument) {
    var element = this[0], settings, staticRules, existingRules, data, param,
        filtered;
    ...
    data =
        $.validator.normalizeRules($.extend({}, $.validator.classRules(element),
...
$.extend($.expr.pseudos || $.expr[":"], {
  // '|| $.expr[ ":" ]' here enables backwards compatibility to jQuery 1.7. Can
  // be removed when dropping jQ 1.7.x support
...
$.extend($.validator, {
  ...
  defaults : {
    ...
    onkeyup : function(element, event) {
      ...
      if (event.which === 9 && this.elementValue(element) === "" ||
        ...
        return;
      } else if (element.name in this.submitted || element.name in
                                                       this.invalid) {
        this.element(element);
      }
    },
  ...
  setDefaults : function(
      settings) { $.extend($.validator.defaults, settings); },
  ...
  prototype : {
    ...
    init : function() {
      ...
      $.each(
          rules,
          function(key,
                   value) { rules[key] = $.validator.normalizeRule(value); });
    ...
    element : function(element) {
      var cleanElement = this.clean(element),
          checkElement = this.validationTargetFor(cleanElement), v = this,
    ...
    showErrors : function(errors) {
      if (errors) {
        ...
        this.successList =
            $.grep(this.successList,
                   function(element) { return !(element.name in errors); });
      }
    ...
    check : function(element) {
      ...
      var rules = $(element).rules(),
          rulesCount = $.map(rules, function(n, i) { return i; }).length,
          dependencyMismatch = false, val = this.elementValue(element), result,
          method, rule, normalizer;

      // Prioritize the local normalizer defined for this element over the
      // global one if the former exists, otherwise user the global one in case
      // it exists.
      ...
      // If normalizer is defined, then call it to retreive the changed value
      // instead of using the real one. Note that `this` in the normalizer is
      // `element`.
      if (normalizer) {
        ...
        // Delete the normalizer from rules to avoid treating it as a
        // pre-defined method.
        ...
        try {
          ...
          if (this.settings.debug && window.console) {
            console.log("Exception occurred when checking element " +
                            element.id + ", check the '" + rule.method +
                            "' method.",
                        e);
          }
    ...
    // return the generic message if present and no method specific message is
    // present
    ...
    // The second parameter 'rule' used to be a string, and extended to an
    // object literal of the following form: rule = {
    //     method: "method name",
    ...
    validElements :
        function() { return this.currentElements.not(this.invalidElements()); },
    ...
    showLabel : function(element, message) {
      var place, group, errorID, v,
          error = this.errorsFor(element), elementID = this.idOrName(element),
      ...
      if (error.length) {
        ...
        if (error.is("label")) {
          ...
          // If the element is not a child of an associated label, then it's
          // necessary to explicitly apply aria-describedby
        } else if (error
                       .parents("label[for='" + this.escapeCssMeta(elementID) +
                                "']")
                       .length === 0) {
          ...
          if (!describedBy) {
            describedBy = errorID;
          } else if (!describedBy.match(new RegExp(
          ...
          // If this element is grouped, then assign to all elements in the same
          // group
    ...
    checkable : function(
        element) { return (/radio|checkbox/i).test(element.type); },
    ...
    dependTypes : {
      "boolean" : function(param) { return param; },
      "string" : function(param,
                          element) { return !!$(param, element.form).length; },
  ...
  attributeRules : function(element) {
    var rules = {}, $element = $(element), type = element.getAttribute("type"),
        method, value;
  ...
  methods : {
    ...
    email : function(value, element) {
      ...
      // If you have a problem with this implementation, report a bug against
      // the above spec Or use custom methods to implement your own email
      // validation
    ...
    digits : function(
        value,
        element) { return this.optional(element) || /^\d+$/.test(value); },
    ...
    min : function(value, element,
                   param) { return this.optional(element) || value >= param; },
    ...
    step : function(value, element, param) {
      var type = $(element).attr("type"),
          ...
          toInt = function(
              num) { return Math.round(num * Math.pow(10, decimals)); },
          valid = true, decimals;

      // Works only for text, number and range input types
      // TODO find a way to support input types date, datetime, datetime-local,
      // month, time and week
      if (notSupported) {
    ...
    equalTo : function(value, element, param) {
      ...
      if (this.settings.onfocusout &&
        ...
        target.addClass("validate-equalTo-blur")
            .on("blur.validate-equalTo", function() { $(element).valid(); });
      }
    ...
    remote : function(value, element, param, method) {
      ...
      var previous = this.previousValue(element, method), validator, data,
          optionDataString;
      ...
      $.ajax($.extend(true, {
        ...
        success : function(response) {
          var valid = response === true || response === "true", errors, message,
              submitted;
          ...
          if (valid) {
            ...
            message =
                response || validator.defaultMessage(
                                element, {method : method, parameters : value});
